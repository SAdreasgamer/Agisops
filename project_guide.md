...
### Tech Stack: Spring Boot 4.1.0 (Java 25) + Python 3.13 Hybrid Architecture
...
|---|---|---|
| **Infrastructure Backbone** | Spring Boot 4.1.0 (Java 25 LTS) | Enterprise-grade APIs, Spring Security 7, native gRPC support, first-class Kafka/Temporal support, virtual threads |
| **AI/ML Services** | Python 3.13 | LangGraph 1.2.5, LangChain, sentence-transformers 5.x — the AI/ML ecosystem is Python-first |
| **Cross-Service Communication** | Kafka + Spring gRPC (Protobuf) | Language-agnostic contracts, native Spring Boot 4.1 gRPC auto-configuration, type-safe |
...
|---|---|---|---|
| **API Gateway** | Java 25 | Spring Cloud Gateway | Enterprise routing, filters, rate limiting |
| **Auth Service** | Java 25 | Spring Boot 4.1 + Spring Security 7.1 | OAuth2/OIDC/SAML battle-tested |
| **Workflow Orchestrator** | Java 25 | Spring Boot 4.1 + Temporal Java SDK 1.35.0 | First-class Temporal support, typed workflows, virtual threads |
| **Ingestion Service** | Java 25 | Spring Boot 4.1 + Spring Kafka | Robust Kafka consumer/producer |
| **Notification Service** | Java 25 | Spring Boot 4.1 + Spring WebSocket | WebSocket, Mail, Slack integrations |
| **Verification Service** | Java 25 | Spring Boot 4.1 | Business logic, safety scoring, policy checks |
| **Agent Runtime** | Python 3.13 | LangGraph 1.2.5 + FastAPI | LangGraph is Python-only, no Java equivalent |
| **Retrieval Service** | Python 3.13 | FastAPI | sentence-transformers 5.x, chunking libs, reranking |
| **Memory Service** | Python 3.13 | FastAPI | Tightly coupled with agent runtime + vector ops |
| **Embedding Workers** | Python 3.13 | Celery / Kafka consumer | Batch embedding, tiktoken, chunking |

### Cross-Language Communication

```
┌──────────────────┐         ┌──────────────────┐
│  Spring Boot 4.1 │  gRPC   │  Python           │
│  Services        │◄───────►│  AI Services      │
│  (Native Spring  │ Protobuf│                    │
│   gRPC)          │         │  (Python 3.13)    │
│  (Java 25 LTS)   │         │                    │
└────────┬─────────┘         └────────┬───────────┘
         │                            │
...
2. **Spring gRPC (native)** — Synchronous request-response when needed (e.g., API gateway calling retrieval service for search). Spring Boot 4.1's native gRPC auto-configuration with `@GrpcAdvice` exception handling and built-in observability. Shared `.proto` files ensure type safety across Java and Python.
...
The AI/ML ecosystem (LangGraph 1.2.5, LangChain, sentence-transformers 5.x, tiktoken) is **Python-first** with no mature Java equivalents. Forcing Java here means reinventing wheels or using immature libraries. Meanwhile, Spring Boot 4.1 is the gold standard for enterprise Java services — now with native gRPC support, built-in resilience annotations, and virtual thread integration.
...
5. **Every service emits OpenTelemetry traces** — Micrometer Tracing (Java) + opentelemetry-python
6. **Contract-first API design** — Protobuf for inter-service (native Spring gRPC), OpenAPI for external
...
| Rate Limiting | Per-user, per-endpoint limits (native Spring Boot 4 resilience) |
| Request Tracing | Inject trace ID (Micrometer Tracing + OpenTelemetry) |
| Tenant Routing | Multi-tenant request isolation |

**Technology:** Spring Cloud Gateway 2025.1.2 + Spring Security 7.1
...
**Technology:** Spring Boot 4.1 + Spring Security 7.1 OAuth2 Resource Server
...
**Technology:** Spring Boot 4.1 + Temporal Java SDK 1.35.0
...
**Technology:** Spring Boot 4.1 + Spring Kafka
...
**Technology:** Spring Boot 4.1 + Spring WebSocket + Spring Mail
...
**Technology:** Spring Boot 4.1 (business logic, no AI dependencies needed)
...
**Technology:** Python 3.13 + LangGraph 1.2.5 + FastAPI
...
**Why Python is non-negotiable here:** LangGraph 1.2.5, LangChain, and the entire multi-agent orchestration ecosystem are Python-only. Spring AI / LangChain4j do not support stateful agent graphs with conditional routing, supervisor patterns, or reflection loops.
...
**Technology:** Python 3.13 + FastAPI + sentence-transformers 5.x + pgvector
...
**Technology:** Python 3.13 + FastAPI + Redis + pgvector
...
### Resilience Pattern (Spring Boot 4 — Native Annotations)

Spring Boot 4 introduced native resilience annotations, eliminating the need for external Resilience4j starters:
...
public class LLMGatewayService {
    ...
    @ConcurrencyLimit(name = "llm-provider", limit = 10)
    public String callLLM(String prompt, String model) {
        return llmClient.generate(prompt, model);
    }

    @Recover
    public String fallbackLLMCall(Throwable t, String prompt, String model) {
...
```yaml
# application.yml — Spring Boot 4 native resilience config
spring:
  retry:
    enabled: true
  concurrency:
...
│       └── common.proto
│
├── java-services/                       # Spring Boot 4.1 services (Java 25 LTS)
...
|---|---|---|---|
| **Phase 1** | Foundation & Infrastructure | 5-6 weeks | Maven multi-module, Docker Compose, Flyway, Kafka, PostgreSQL, Spring Boot 4.1 |
| **Phase 2** | Core AI Agent Pipeline | 4-5 weeks | LangGraph 1.2.5, FastAPI, native Spring gRPC, sentence-transformers 5.x, pgvector |
| **Phase 3** | Workflow Orchestration & Dashboard | 5-6 weeks | Temporal Java SDK 1.35.0, Spring WebSocket, Next.js |
...
- [ ] Configure parent POM with:
  - Java 25 LTS, Spring Boot 4.1.0, Spring Cloud 2025.1.2
  - Shared dependencies: Spring Data JPA, Spring Kafka, Micrometer Tracing, Lombok, MapStruct (Resilience4j replaced by native Spring Boot 4 resilience annotations)
...
> Architected and built **AEGISOPS**, an enterprise-grade AI-native engineering operations platform using a **polyglot microservices architecture (Spring Boot 4.1 + Python 3.13)**. Spring Boot (Java 25 LTS) handles API gateway, OAuth2/RBAC (Spring Security 7), workflow orchestration (Temporal 1.35.0), and event ingestion (Spring Kafka). Python services power multi-agent investigation (LangGraph 1.2.5), hybrid RAG pipeline (pgvector + sentence-transformers 5.x), and autonomous verification. Cross-service communication via **Apache Kafka + native Spring gRPC (Protobuf)**. Reduced simulated MTTR by 60%+ through cross-system incident correlation. Deployed on Kubernetes with OpenTelemetry observability and multi-tenant isolation.

### What This Demonstrates

| Skill Area | Tech Used |
|---|---|
| **Backend Engineering** | Spring Boot 4.1, Spring Data JPA, Spring Cloud Gateway 2025.1.2, REST, native Spring gRPC |
| **AI Systems Engineering** | LangGraph 1.2.5, RAG pipeline, multi-agent orchestration |
| **Distributed Systems** | Apache Kafka, Temporal workflows 1.35.0, saga pattern |
| **Polyglot Architecture** | Java ↔ Python via Protobuf contracts + Kafka |
| **Security** | Spring Security 7.1, OAuth2, RBAC, JWT, mTLS |
| **Kubernetes & DevOps** | Helm, HPA, Terraform, GitHub Actions CI/CD |
| **Observability** | Micrometer Tracing, OpenTelemetry, Prometheus, Grafana |

### Target Roles

| Role | Strongest Sections |
|---|---|
| **Backend Engineer (Java)** | Spring Boot 4.1 microservices, Kafka, Temporal, Spring Security 7.1 |
...
Using Spring Boot 4.1 + Python 3.13 is not a compromise — it's the **optimal architecture** for this problem domain:

- Spring Boot 4.1 excels at: typed APIs, enterprise security (Spring Security 7), native gRPC, Kafka consumers, workflow orchestration, virtual threads
- Python 3.13 excels at: AI/ML agent frameworks (LangGraph 1.2.5), embedding pipelines (sentence-transformers 5.x), NLP tooling
- The boundary between them (Kafka + native Spring gRPC) is clean, well-defined, and industry-standard
...
| **Cross-Language Integration** | Protobuf contracts, native Spring gRPC, Kafka serialization |
| **Security** | Spring Security 7.1, OAuth2, RBAC, audit logging |
| **Observability** | Micrometer Tracing (Java) + opentelemetry (Python) unified in Grafana |
...
| Spring Boot cold start time | Use Spring Boot 4.x AOT, CDS (Class Data Sharing), or GraalVM native image for lightweight services |
